...
const ProductModal = ({ product, onClose, onSave }) => {
  ...
  return (
    ...
    >
      <motion.div
        style={{
          background: 'linear-gradient(135deg, #13131f, #0d0d18)',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 800, color: '#E8E8F0', margin: 0 }}>
          ...
          <button
            onClick={onClose}
            style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'rgba(232,232,240,0.5)' }}
          >
            <MdClose size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
              <div style={{ width: 80, height: 80, borderRadius: 16, background: 'rgba(212,175,55,0.08)', border: '1px solid rgba(212,175,55,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', fontSize: 36, flexShrink: 0 }}>
                {preview ? (
                  <img
                    src={preview}
                    alt="preview"
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                  <button
                    ...
                    style={{
                      ...
                      background: imageMode === 'upload' ? 'rgba(212,175,55,0.2)' : 'rgba(255,255,255,0.05)',
                      border: imageMode === 'upload' ? '1px solid rgba(212,175,55,0.5)' : '1px solid rgba(255,255,255,0.1)',
                      color: imageMode === 'upload' ? '#D4AF37' : 'rgba(232,232,240,0.5)',
                  ...
                  <button
                    ...
                    style={{
                      flex: 1, padding: '8px 0', borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: 'pointer',
                      background: imageMode === 'url' ? 'rgba(212,175,55,0.2)' : 'rgba(255,255,255,0.05)',
                      border: imageMode === 'url' ? '1px solid rgba(212,175,55,0.5)' : '1px solid rgba(255,255,255,0.1)',
                      color: imageMode === 'url' ? '#D4AF37' : 'rgba(232,232,240,0.5)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                    }}
                  >
                ...
                {/* Upload File */}
                {imageMode === 'upload' && (
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 14px', borderRadius: 10, background: 'rgba(212,175,55,0.08)', border: '1px solid rgba(212,175,55,0.25)', color: '#D4AF37', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                    <MdImage size={16} />
                    {imageFile ? imageFile.name.substring(0, 20) + '...' : 'Choose Image File'}
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            >
              {CATEGORIES.map((c) => (
                <option key={c} value={c} style={{ background: '#13131f', color: '#E8E8F0' }}>{c}</option>
              ))}
            </select>
          </div>
    ...
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <motion.button
          ...
          className="btn-royal self-start"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        <AnimatePresence>
          {filtered.map((product) => (
            <motion.div
              key={product._id}
              className="royal-card royal-card-hover p-4"
              layout
      ...
      <AnimatePresence>
        {deleteConfirm && (
          ...
          >
            <motion.div
              style={{ background: 'linear-gradient(135deg, #13131f, #0d0d18)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 24, padding: 28, maxWidth: 380, width: '100%', textAlign: 'center' }}
            ...
            >
              ...
              <p style={{ fontSize: 14, color: 'rgba(232,232,240,0.45)', marginBottom: 24 }}>
                ...
                <strong style={{ color: '#E8E8F0' }}>{deleteConfirm.name}</strong>?
              </p>
              <div style={{ display: 'flex', gap: 12 }}>
                <button
